...
Giao diện desktop dùng **Qt Widgets với QSS nhẹ**, phong cách dashboard công cụ vận hành: nền trung tính sáng, một màu nhấn xanh lam đậm, typography rõ cấp bậc, khoảng trắng nhất quán, thẻ thông tin có viền nhẹ và các trạng thái server có cả nhãn chữ lẫn màu. Ưu tiên bảng dữ liệu dễ quét hơn hiệu ứng trang trí; không lạm dụng gradient, đổ bóng hoặc biểu đồ giả. Bố cục gồm thanh tiêu đề/tác vụ, cột thiết lập server và vùng chính hiển thị tiến độ, bảng server, dòng sự kiện; cấu hình nâng cao được thu gọn để người dùng mới không phải hiểu chunk/timeout trước khi tải.
...
Checkpoint sidecar gắn với `<output>.part`, gồm phiên bản định dạng, tên/kích thước/SHA-256 file nguồn, kích thước chunk và danh sách chunk đã hoàn thành cùng SHA-256 từng chunk. Không lưu dữ liệu nhị phân trong JSON; ghi sidecar theo lối atomic replace và không cho ghi đồng thời từ nhiều worker. Chỉ đánh dấu chunk hoàn tất trong checkpoint sau khi byte của chunk đã ghi xong và đồng bộ xuống đĩa. Nếu ứng dụng/crash xảy ra trước commit checkpoint, chunk đó phải tải lại; không bao giờ tin chunk chỉ vì có byte trong `.part`.

Khi mở lại, luôn lấy metadata từ server theo quy tắc đồng thuận trước. Chỉ dùng checkpoint nếu metadata, chunk size và kích thước `.part` trùng. Đọc và hash lại từng chunk ghi trong checkpoint trước khi bỏ qua tải; chunk sai hoặc thiếu bị đưa lại hàng đợi. Nếu checkpoint thiếu, hỏng định dạng, khác phiên bản nguồn hoặc `.part` thiếu/sai kích thước, tạo phiên mới và báo rõ dữ liệu cũ sẽ không được tái dùng; không nhận file cũ là hoàn chỉnh. Không ghi đè output đã tồn tại khi người dùng chưa chọn hành động thay thế rõ ràng. Một output chỉ có một phiên tải tại một thời điểm. CLI và GUI dùng cùng quy tắc checkpoint nếu chạy download với chế độ khôi phục; đường CLI cũ mặc định vẫn giữ hành vi tương thích nếu chưa chọn chế độ đó.
